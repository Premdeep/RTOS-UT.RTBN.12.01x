...
// **write this function**
			uint16_t offset = (512 * sector);
		uint32_t saddr = (EDISK_ADDR_MIN + offset);
		if(saddr > EDISK_ADDR_MAX)
			return RES_PARERR;
		else{
			int8_t* ptr = (int8_t*)saddr;
...
// **write this function**
		int result = 0;
		uint32_t saddr = EDISK_ADDR_MIN + (512)*sector;
		if(saddr > EDISK_ADDR_MAX)
			return RES_PARERR;
		else{
			uint32_t* src = (uint32_t*)buff;
			for(uint8_t i = 0; i < 4; i++){
				uint8_t temp = Flash_FastWrite(src, saddr, 32);
				src = (src + 32);
				saddr = (saddr + 32*4);
				result += temp;
				
			}
		//	result=Flash_FastWrite((uint32_t*)buff, saddr, 128);
...
// **write this function**
	uint32_t start = EDISK_ADDR_MIN;
	//enum DRESULT res;
	while(start <= EDISK_ADDR_MAX){
		//res = Flash_Erase(start);
		if(Flash_Erase(start) == RES_ERROR)
			return RES_ERROR;
		start+=1024;
	}	
//	return res;
